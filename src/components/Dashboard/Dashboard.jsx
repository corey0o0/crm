...
const Dashboard = () => {
  ...
  const [saveTimeout1, setSaveTimeout1] = useState(null);
  const [saveTimeout2, setSaveTimeout2] = useState(null);
  ...
  // 메모 저장
  const saveMemos = async (memo1Content, memo2Content) => {
    try {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      safeRecentServices.forEach(service => {
        ...
        // ALL에 모든 데이터 추가
        processedRecentServices.ALL.push(processedService);

        // 브랜드별로 데이터 추가
        if (service.brand === 'XRB') {
          processedRecentServices.XRB.push(processedService);
        } else if (service.brand === 'NBK') {
          processedRecentServices.NBK.push(processedService);
        }
      });
